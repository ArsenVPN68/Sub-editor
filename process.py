...
def process_clash(urls, custom_name, repo_info):
    ...
    for url in urls:
        ...
        if 'proxies:' in content or 'proxies:' in decoded_content:
            ...
            text_to_parse = decoded_content if decoded_content else content
            found_links = re.findall(r'(?:vless|vmess|trojan|ss|hy2|tuic)://[^\s"#]+(?:#[^\s"]*)?', text_to_parse)
            
            if not found_links:
                found_links = text_to_parse.splitlines()
