...
def fetch_content(url):
    """دریافت داتای ساب با هدر کاربر واقعی"""
...
def get_next_filename(folder_path, prefix="sub", extension=".txt"):
    """نام‌گذاری خودکار فایل جدید بدون خراب کردن قبلی‌ها"""
    count = 1
    while True:
...
def process_raw(url, custom_name):
    ...
    result_text = "\n".join(new_lines)
    encoded_result = base64.b64encode(result_text.encode("utf-8")).decode(
        "utf-8"
    )
    ...
    print(f"✅ ساب RAW جدید ساخت شد: {filepath}")


# ==========================================
# ۲. پردازش CLASH (YAML بدون دستکاری کانفیگ‌ها)
# ==========================================
def process_clash(url, custom_name):
    ...
    # فقط اسم تغییر می‌کنه، بقیه دیکشنری کاملاً دست‌نخورده باقی می‌مونه
    ...
    print(f"✅ ساب Clash جدید ساخت شد: {filepath}")
...
def process_json(url, custom_name):
    ...
    if isinstance(outbounds, list):
        # تغییر تگ (اسم) بدون دست زدن به پورت و بقیه ساختارها
        for item in outbounds:
            if isinstance(item, dict) and "tag" in item:
                # حذف گروه یا فرعی‌های خاص در صورت نیاز اما حفظ کامل پروتکل
                old_tag = item["tag"]
    ...
    print(f"✅ ساب JSON جدید ساخت شد: {filepath}")


# ==========================================
# دریافت پارامترها از GitHub Action
# ==========================================
