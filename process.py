...
def fetch_content(url):
    """دریافت محتوای ساب با هدر سفارشی"""
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "v2rayNG/1.8.12 (com.v2ray.ang; build 500; Android 13)"
        },
    )
    try:
...
def parse_uri_to_clash(link):
    """استخراج تمام جزییات شبکه جهت جلوگیری از قطعی سرورها"""
    try:
        link = link.strip()
        if not link:
            return None

        # ۱. VLESS (پشتیبانی کامل از Reality, TLS, WS, gRPC)
        ...
        # ۲. VMESS (پشتیبانی کامل از WS, gRPC, TLS)
...
# ==========================================
# ۲. پردازش CLASH (کاملاً استاندارد و هوشمند)
# ==========================================
def process_clash(urls, custom_name, repo_info):
    ...
    for url in urls:
        ...
        # دکود محتوا در صورت بیس۶۴ بودن ساب ورودی
        decoded_content = safe_b64decode(content)
        
        # اگر مستقیم فایل کلش یا YAML بود
        if 'proxies:' in content or 'proxies:' in decoded_content:
            ...
            # اگر محتوا لینک‌های vless/vmess/trojan بود (مثل ساب‌های BPB)
            lines = decoded_content.splitlines()
            for line in lines:
                proxy = parse_uri_to_clash(line)
                if proxy:
    ...
    # پیکربندی استاندارد و قوی کلش به همراه DNS حل‌کننده تحریم
    clash_config = {
        'port': 7890,
        'socks-port': 7891,
        'allow-lan': True,
